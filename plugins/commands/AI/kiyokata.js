...
export const config = {
  name: "kiyokata",
  version: "1.0.0",
  credits: "Original Idea from: Akhiro",
  usage: "[prompt]",
  cooldown: 5
};
...
export async function onCall({ message, args }) {
  try {
    ...
    if (query) {
      message.react("⏳");
      const processingMessage = await message.reply(`🤵 | Kiyotaka AI, please wait a moment...`);

      const apiUrl = `https://lianeapi.onrender.com/@public/api/kiyokata?key=j86bwkwo-8hako-12C&query=${encodeURIComponent(query)}`;
