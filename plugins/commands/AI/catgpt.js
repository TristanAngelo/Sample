...
export async function onCall({ message, args }) {
  try {
    ...
    if (query) {
      message.react("⏳");
      const processingMessage = await message.reply(`😾 | CatGPT, please wait a moment...`);
      ...
      if (response.data && response.data.message) {
        ...
        console.log(`Sent CatGPT response to the user`);
      } else {
    ...
    console.error(`❌ | Failed to get CatGPT response: ${error.message}`);
